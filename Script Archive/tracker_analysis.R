...
# TO DO
# 2023-01-14 Add id column to each from name of imported csv starting from 5, so full
# non summarized output can be QCd
...
# Load movement data to join tank number with tracker data
...
tanks <- trials2021_Q %>%
  dplyr::select(tank, urchinID) %>%
  mutate(ID = urchinID) %>%
  dplyr::select(-urchinID) %>%
  distinct()

Tracker_summary <- Tracker_summary %>%
  left_join(tanks, by = "ID")
Tracker_summary <- Tracker_summary %>%
  mutate(Alg = factor(Alg),
         Pyc = factor(Pyc),
...
M001 <- as.ltraj(xy = All_move_non_na[, c('x','y')], typeII = FALSE, id = All_move_non_na$id)
plot(M001)
...
summary(lmer(distance ~ Pyc + Alg + Urchin + (1 | tank), data = Tracker_summary))
